...
then
    ...
    echo -e "$G now you are in root acess $N" &>>$LOG_FILE
fi
...
if [ $? -ne 0 ]          
then
    echo "MYSQL is not installed... need to install it" &>>$LOG_FILE
    dnf install mysql -y
    VALIDATE $? "mysql"        #-- validate $?=0 --> install MYSQL.... SUCCESS
else
    echo -e "$M MYSQL is already installed..... $N" &>>$LOG_FILE
fi

dnf list installed python3 &>>$LOG_FILE
if [ $? -ne 0 ]
then
    echo "python3 is not installed... need to install it" &>>$LOG_FILE
    dnf install python3 -y
    VALIDATE $? "python3"
else  
    echo -e "$M python3 already installed..... $N" &>>$LOG_FILE
fi

dnf list installed nginx &>>$LOG_FILE
if [ $? -ne 0 ]
then 
    echo "nginx is not installed... need to install it" &>>$LOG_FILE
    dnf install nginx -y
    VALIDATE $? "nginx"
else
    echo -e "$M nginx already installed..... $N" &>>$LOG_FILE
fi
